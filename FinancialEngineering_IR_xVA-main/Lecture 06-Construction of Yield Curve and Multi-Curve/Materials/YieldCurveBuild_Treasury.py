...
def YieldCurve(instruments, maturities, r0, method, tol):
    r0 = deepcopy(r0)
    ri = MultivariateNewtonRaphson(
        r0, maturities, instruments, method, tol=tol)
    return ri
...
def EvaluateInstruments(ti, ri, instruments, method):
    def P0Ttemp(t): return P0TModel(t, ti, ri, method)
...
def linear_interpolation(ti, ri):
    def interpolator(t): return np.interp(t, ti, ri)
    return interpolator
...
def mainCode():
    ...
    def swap1(P0T): return IRSwap(OptionTypeSwap.PAYER,
                                  1, K[0], 0.0, 0.0, mat[0], 4*mat[0], P0T)

    def swap2(P0T): return IRSwap(OptionTypeSwap.PAYER,
                                  1, K[1], 0.0, 0.0, mat[1], 4*mat[1], P0T)

    def swap3(P0T): return IRSwap(OptionTypeSwap.PAYER,
                                  1, K[2], 0.0, 0.0, mat[2], 4*mat[2], P0T)

    def swap4(P0T): return IRSwap(OptionTypeSwap.PAYER,
                                  1, K[3], 0.0, 0.0, mat[3], 4*mat[3], P0T)

    def swap5(P0T): return IRSwap(OptionTypeSwap.PAYER,
                                  1, K[4], 0.0, 0.0, mat[4], 4*mat[4], P0T)

    def swap6(P0T): return IRSwap(OptionTypeSwap.PAYER,
                                  1, K[5], 0.0, 0.0, mat[5], 4*mat[5], P0T)

    def swap7(P0T): return IRSwap(OptionTypeSwap.PAYER,
                                  1, K[6], 0.0, 0.0, mat[6], 4*mat[6], P0T)
    def swap8(P0T): return IRSwap(OptionTypeSwap.PAYER,
                                  1, K[7], 0.0, 0.0, mat[7], 4*mat[7], P0T)
    ...
    def P0T_Initial(t): return P0TModel(t, mat, r0, method)
    def P0T(t): return P0TModel(t, mat, ri, method)
